...
function cal_popup1(str_datetime) {
	...
	var obj_calwindow = window.open("/64VB/html/calendar.html?datetime=" + this.dt_current.valueOf() + "&id=" + this.id, "Calendar", "width=200,height=" + (this.time_comp ? 215 : 190) + ",status=no,resizable=no,top=200,left=200,dependent=yes,alwaysRaised=yes");
...
function cal_popup123(str_datetime, obj_target) {
	this.gen_date = cal_gen_date1;
	this.gen_time = cal_gen_time1;
	this.gen_tsmp = cal_gen_tsmp1;
	this.prs_date = cal_prs_date1;
	...
	calendars[this.id] = this;
	if (str_datetime) {
		this.dt_current = this.cal_prs_tsmp123(str_datetime);
	} else {
		this.dt_current = this.prs_tsmp(this.target.value);
		this.dt_selected = this.dt_current;
	}
...
function cal_prs_tsmp123(str_datetime) {
	// if no parameter specified return current timestamp
	if (!str_datetime) {
		return (new Date());
	}
